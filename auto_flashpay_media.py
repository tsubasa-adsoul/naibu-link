...
class LinkAnalyzerApp:
    def __init__(self):
        self.root = ctk.CTk()
        self.root.title("🔗 FlashPay 内部リンク分析ツール（全自動版）")
    ...
    def setup_ui(self):
        ...
        title_label = ctk.CTkLabel(self.main_frame, text="🔗 FlashPay 内部リンク分析ツール（全自動版）",
                                   font=ctk.CTkFont(size=32, weight="bold"))
        title_label.pack(pady=20)

        self.url_entry = ctk.CTkEntry(self.main_frame, placeholder_text="https://flashpay.jp/media/", width=500)
        self.url_entry.pack(pady=10)
        self.url_entry.insert(0, "https://flashpay.jp/media/")
    ...
    def extract_links(self, soup):
        selectors = ['.entry-content', '.post-content', 'article', 'main', '.content']
    ...
    def is_content(self, url):
        ...
        # /media/ 直下の記事 または トップページ
        if path == '/media':
            return True
        return path.startswith('/media/') and re.match(r'^/media/[a-z0-9\-]+$', path)
        ...
        try:
            ...
            while to_visit and len(pages) < 500:
                ...
                try:
                    ...
                    # flashpay | フラッシュペイ などのサイト名を除去
                    title = re.sub(r'\s*[|\-]\s*.*(flashpay|フラッシュペイ).*$', '', title, flags=re.IGNORECASE)
    ...
    def auto_export_csv(self):
        ...
        try:
            ...
            # CSVファイル名設定
            csv_filename = f"flashpay-{date_folder}.csv"
