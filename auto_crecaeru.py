...
SITES_CONFIG = {
    ...
    "クレかえる": {
        "base_url": "https://crecaeru.co.jp",  # ← これが間違っていた
...
def is_content_url_crecaeru(url, base_domain):
    """クレかえる専用のURL判定ロジック"""
    try:
...
def crawl_site(site_name, base_url, max_pages=100):
    ...
    # サイト別のURL判定関数を選択
    if site_name == "クレかえる":
        is_content_func = is_content_url_crecaeru
    else:
        is_content_func = is_content_url_generic
    ...
    # 手動でもいくつかの基本URLを追加
    manual_urls = [base_url]
    if site_name == "クレかえる":
        manual_urls.extend([
            f"{base_url}/category/amazonギフト券",
            f"{base_url}/category/iTunesギフトカード", 
            f"{base_url}/category/nanacoギフト",
            f"{base_url}/blog"
        ])
    
    initial_urls = list(set(manual_urls + list(sitemap_urls)))
    st.info(f"サイトマップから{len(sitemap_urls)}個のURL、手動追加{len(manual_urls)}個、合計{len(initial_urls)}個のURLを発見")
    
    # コンテンツURLのみを抽出
    to_visit = []
    for url in initial_urls:
        if is_content_func(url, domain):
            to_visit.append(url)
    
    to_visit = to_visit[:max_pages]
    
    if not to_visit:
        st.error("クロール対象のURLが見つかりませんでした")
        
        # デバッグ情報を表示
        st.subheader("デバッグ情報")
        st.write(f"ドメイン: {domain}")
        st.write(f"初期URL数: {len(initial_urls)}")
        st.write("初期URL例:")
        for url in initial_urls[:10]:
            is_valid = is_content_func(url, domain)
            st.write(f"- {url} ({'有効' if is_valid else '無効'})")
    ...
    for i, url in enumerate(to_visit):
        ...
        try:
            ...
            # サイト名を除去
            if site_name == "クレかえる":
                title = re.sub(r'\s*[|\-].*(crecaeru|クレかえる).*
...
if __name__ == "__main__":
    main()  # カテゴリページング
        ...
        # 除外パターン
        exclude_patterns = [
            r'/sitemap', r'sitemap.*\.(xml|html)', r'/page/\d+', 
            r'/site/', r'/wp-', r'/tag/', r'/wp-content', r'/wp-admin', 
            r'/wp-json', r'#', r'\?utm_', r'/feed/', r'mailto:', r'tel:', 
            r'/privacy', r'/terms', r'/contact', r'/about', r'/company',
            r'/go-', r'/redirect', r'/exit', r'/out', r'/search',
...
def is_content_url_generic(url, base_domain):
    """汎用URL判定ロジック"""
    try:
        path = urlparse(url).path.lower()
...
if __name__ == "__main__":
    main(), r'/wp-admin/', r'/wp-content/', r'/feed/', 
            r'\.(jpg|png|pdf|zip)
...
if __name__ == "__main__":
    main(), '', title, flags=re.IGNORECASE).strip()
            else:
                title = re.sub(r'\s*[|\-].*
...
if __name__ == "__main__":
    main(), '', title).strip()
            ...
            for link_data in extracted_links:
                ...
                # 内部リンクかチェック
                link_domain = urlparse(normalized_link).netloc.lower().replace('www.', '')
                if link_domain == domain and is_content_func(normalized_link, domain):
                    all_links.append({
                        'source_url': url,
                        'source_title': title,
                        'target_url': normalized_link,
